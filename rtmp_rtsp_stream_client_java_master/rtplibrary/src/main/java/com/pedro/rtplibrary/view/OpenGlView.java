...
import static com.pedro.encoder.input.gl.render.ManagerRender.numFilters;
...
public class OpenGlView extends OpenGlViewBase {
    ...
    private int[] typePos = new int[numFilters];
...
//            ManagerRender.numFilters = typedArray.getInt(R.styleable.OpenGlView_numFilters, 1);
    ...
    ///
    public void ogvInit(){
        for(int i=0; i<numFilters; i++) typePos[i] = -1;
    }
    ...
    public void setFilters( BaseFilterRender baseFilterRender,BaseFilterRender baseFilterRender2) {
        filterQueue.add(new Filter(0, baseFilterRender));
        filterQueue.add(new Filter(1, baseFilterRender2));
    }
    ...
    public void removeFilter(int type){
        if(typePos[type] == -1) return;
        if((--filterNum) < -1) filterNum = -1;
        Log.d("numf","remove filter / filterNum : " + filterNum);
        managerRender.removeFilter(typePos[type]);
        for(int i=0; i<numFilters; i++){
            if(typePos[i] > typePos[type] && i != type) typePos[i]--;
        }
        typePos[type] = -1;
    }
    ...
    ///////////////
    @Override
    public void setFilterT(int type, BaseFilterRender baseFilterRender){
        if((++filterNum) >= numFilters) filterNum = numFilters-1;
        Log.d("numf","type : " + type+ " ; filterNum : "+filterNum+"/"+"numFilters");
        typePos[type] = filterNum;
        setFilter(filterNum, baseFilterRender);
    }
