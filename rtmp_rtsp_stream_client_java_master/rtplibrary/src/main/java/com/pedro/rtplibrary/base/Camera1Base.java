 ...
 * camera1 api 및 마이크와 함께 스트리밍하기 위한 wrapper.
 * SurfaceView, TextureView, OpenGlView (OpenGl을 사용하는 Custom SurfaceView)를 통한 스트리밍 지원.
 * SurfaceView 및 TextureView는 H264를 위한 buffer to buffer 인코딩 모드를 사용하고
 * OpenGlView는 surface to buffer 모드를 사용.
 * surface to buffer mode는 일반적으로 버퍼 처리를 건너 뛰기 때문에 더 좋음.
 *
 * API requirements:
